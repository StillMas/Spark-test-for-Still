...
object Students_t_EM extends Serializable {
  def main(args:Array[String]): Unit = {
    ...
    val mu=5.0
    val Sigma=1.0
    val v=6  //degree of freedom
    /**
    //Choice1:Generate From StudentsT distribution
    @transient val RanStudentsT=new StudentsT(v);@transient val Random=ThreadLocalRandom.current()
    @transient val RandNum=RanStudentsT.sample(N).map(i=>i+mu)
    */

    //Choice2:Generate From Normal and Gamma Distribution
    @transient val RandChiSq=new ChiSquared(v);@transient val Random=ThreadLocalRandom.current()
    @transient val RandNum=Array.ofDim[Double](N)
    for (i<-1 until N) {RandNum(i)=Random.nextGaussian()*Sigma/math.sqrt(RandChiSq.draw()/v)+mu}
    ...
    println("final",Estmu,EstSigma,Estv,SigmaFinal,ii)
  }
}

...
/**
  * Created by Still on 2017/6/2.
  */
object Logistic_Newton {
  def main(args:Array[String]) {
    val conf=new SparkConf().setAppName("Logistic_Newton").setMaster("local")
    ...
    //val data=(1 to n).map(i=>1-(1/(1+math.exp(ranbinom.draw())))).toArray
    ...
    var Oldbeta=DenseVector.ones[Double](k);var beta=DenseVector.zeros[Double](k);var ii=0
    val epsilon=0.01;var L2norm=0.0
    do {
      ii+=1
      Oldbeta = beta
